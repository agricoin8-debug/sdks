...
async function main() {
    try {
        ...
        // Normalize hex strings: add '0x' prefix if missing for non-array fields
        const normalizeHex = (value: string | string[]): `0x${string}` | `0x${string}`[] => {
            if (Array.isArray(value)) {
                return value.map(v => (v.startsWith('0x') ? v : `0x${v}`) as `0x${string}`);
            }
            return (value.startsWith('0x') ? value : `0x${value}`) as `0x${string}`;
        };

        const normalizedRegisters: WorkloadMeasurementRegisters = {
            tdattributes: normalizeHex(registers.tdattributes) as `0x${string}`,
            xfam: normalizeHex(registers.xfam) as `0x${string}`,
            mrtd: normalizeHex(registers.mrtd),
            mrconfigid: normalizeHex(registers.mrconfigid) as `0x${string}`,
            rtmr0: normalizeHex(registers.rtmr0),
            rtmr1: normalizeHex(registers.rtmr1) as `0x${string}`,
            rtmr2: normalizeHex(registers.rtmr2) as `0x${string}`,
            rtmr3: normalizeHex(registers.rtmr3) as `0x${string}`,
        };

        // Expand to all singular register combinations
        const singularRegisters = expandToSingularRegisters(normalizedRegisters);

        console.log('Measurement Registers:');
        console.log('=====================');
        console.log(`mrtd values: ${Array.isArray(normalizedRegisters.mrtd) ? normalizedRegisters.mrtd.length : 1}`);
        console.log(`rtmr0 values: ${Array.isArray(normalizedRegisters.rtmr0) ? normalizedRegisters.rtmr0.length : 1}`);
